...
const edades = persons.reduce((accumulator, person) => {
...
const cochesEspanioles = persons.filter((person) => {
  return person.country === 'ES';
});

console.log(cochesEspanioles);
...
const personasConCoches = persons.map((person) => {
  const personCopy = { ...person };

  const personsCars = cars.find((car) => {
    return personCopy.car === car.id;
  });

  personCopy.coche = personsCars;
  return personCopy;
});

const personasConFord = personasConCoches.filter((car) => {
  return car.coche.brand === 'Ford';
});

console.log(personasConCoches);
